...
class M_po extends CI_Model
{
    ...
    function get_list_po()
    {
        ...
        if (!empty($_POST['search']['value'])) {
            ...
            $query = "SELECT id, kode_supply, nama_supply, kode_pemesan, pemesan, nopo, nopotxt, noppo, noppotxt, no_refppo, tgl_refppo, tglpo, tglpotxt, tglppo, tglppotxt, ket, noreftxt, terbayar, nopp FROM po WHERE batal <> 1 $periode $keyfilter1 AND (noreftxt LIKE '%$keyword%' 
            ...
            $query = "SELECT id, kode_supply, nama_supply, kode_pemesan, pemesan, nopo, nopotxt, noppo, noppotxt, no_refppo, tgl_refppo, tglpo, tglpotxt, tglppo, tglppotxt, ket, noreftxt, terbayar, nopp FROM po WHERE batal <> 1 $periode $keyfilter1 ORDER BY id DESC";
            $count_all = $this->db_logistik_pt->query($query)->num_rows();
            $data_tabel = $this->db_logistik_pt->query($query . " LIMIT $start,$length")->result();
        }
        ...
        foreach ($data_tabel as $hasil) {
            ...
            if ($hasil->terbayar == 0) {
                $row[] = $thisUsr == 'Staff Purchasing' || $thisUsr == 'Dept Head Purchasing' ? '<a href="' . site_url('po/edit_po/' . $id) . '" target="_blank" class="btn btn-info fa fa-edit btn-xs" data-toggle="tooltip" data-placement="top" title="Ubah PO" id="btn_edit_barang"> Ubah
                </a>
                ...
                $row[] = '<a href="' . site_url('po/cetak/' . $hasil->nopotxt . '/' . $id) . '" target="_blank" class="btn btn-primary btn-xs fa fa-print" id="a_print_po"> Cetak
                </a>
                ';
            }
    ...
    function get_list_spp()
    {
        ...
        // SELECT id, noppo, noppotxt, tglppo, noref, noreftxt, tglref, tglppo, tgltrm, kodedept, namadept, ket, pt, kodept, lokasi, status, status2, po, jenis FROM ppo WHERE status2 IN ('1','2') AND noppotxt = '$no_spp' AND noreftxt = '$no_ref_spp' AND po = '0' ORDER BY id DESC, tglisi DESC
            ...
            $query = "SELECT id, noppo, noppotxt, tglppo, noref, noreftxt, kodebartxt, nabar, tglppo, namadept, ket, kodedept, namadept, lokasi, status, status2, po, qty, kodept FROM item_ppo WHERE po = '0' AND (status2 IN ('1','2') $keyfilter1 $jenis_spp) ORDER BY id DESC";
            $count_all = $this->db_logistik_pt->query($query)->num_rows();
            $data_tabel = $this->db_logistik_pt->query($query . " LIMIT $start,$length")->result();
        }
    ...
    function get_list_supplier()
    {
        ...
        if (!empty($_POST['search']['value'])) {
            ...
            $query = "SELECT id, kode, supplier, usaha FROM supplier
                        WHERE (account IS NOT NULL) AND (kode LIKE '%$keyword%'
            ...
            $query = "SELECT id, kode, supplier, usaha FROM supplier WHERE account IS NOT NULL ORDER BY supplier ASC";
            $count_all = $this->db_logistik_pt->query($query)->num_rows();
            $data_tabel = $this->db_logistik_pt->query($query . " LIMIT $start,$length")->result();
        }
    ...
    function simpan_rinci_po()
    {
        ...
        // $datainsert['noppo']            = NULL;
        // $datainsert['noppotxt']         = NULL;
        $datainsert['noppo']            = "0";
        $datainsert['noppotxt']         = "0";
        ...
        $datainsert['namapt']           = $this->session->userdata('pt');
        // $datainsert['no_acc']           = "";
        $datainsert['no_acc']           = $pph;
        ...
        $datainsert['totalbayar']       = $this->input->post('txt_total_pembayaran');
        ...
        $datainsertitem['jumharga']         = $jumharga;
        $datainsertitem['kodept']           = $this->input->post('hidden_kodept');
        $datainsertitem['namapt']           = $this->input->post('hidden_namapt');
        //$datainsertitem['kodept']           = $this->session->userdata('kode_pt');//"0";
        ...
        if (empty($this->input->post('hidden_no_po'))) {
            /*** Jika PO Lokal lebih dari 1,5 juta ***/
            if ($jumharga > 1500000 && $hidden_jenis_spp == "SPPI") {
                return "polokal_melebihi_nilai";
            } else {
            ...
            $query_sum_jumharga = "SELECT SUM(jumharga) as totalharga FROM item_po WHERE nopotxt = '$no_po' AND (kodebartxt = '$kodebar' OR nabar = '$nabar')";
            $sum_jumharga = $this->db_logistik_pt->query($query_sum_jumharga)->row();

            $totalharga = $jumharga + $sum_jumharga->totalharga;
            /*** Check apakah barang sudah pernah ditambahkan pada PO yang sama  ***/
            if ($check_brg->num_rows() > 0) {
                return "kodebar_exist";
            }
            /*** Jika PO Lokal lebih dari 1,5 juta ***/
            else if ($totalharga > 1500000 && $hidden_jenis_spp == "SPPI") {
                return "polokal_melebihi_nilai";
            }
    ...
    function ubah_rinci_po()
    {
        ...
        //   string(2) "20"
        //   ["txt_ket_pengiriman"]=>
        //   string(16) "keterangan kirim"
        //   ["txt_lokasi_pengiriman"]=>
        //   string(12) "lokasi kirim"
        //   ["cmb_lokasi_pembelian"]=>
        //   string(2) "HO"
        //   ["txt_no_penawaran"]=>
        //   string(12) "no penawaran"
        ...
        //   string(7) "3100002"
        //   ["hidden_no_ref_po"]=>
        //   string(25) "EST/BWJ/JKT/03/19/3100002"
        //   ["hidden_kode_brg"]=>
        //   string(15) "102505990000190"
        //   ["hidden_nama_brg"]=>
        //   string(15) "AC SPLIT 1/2 PK"
        ...
        //   string(7) "3100001"
        //   ["hidden_no_ref_po"]=>
        //   string(25) "EST/BWJ/JKT/03/19/3100001"
        //   ["hidden_kode_brg"]=>
        //   string(15) "102505950000045"
        //   ["hidden_nama_brg"]=>
        //   string(14) "ABBOCATH NO.22"
        //   ["hidden_satuan_brg"]=>
        //   string(3) "PCS"
        //   ["hidden_id_po"]=>
        //   string(1) "1"
        ...
        // $dataedit['noppo']            = NULL;
        // $dataedit['noppotxt']         = NULL;
        $dataedit['noppo']            = "0";
        $dataedit['noppotxt']         = "0";
        $dataedit['no_refppo']        = $this->input->post('hidden_no_ref');
        ...
        $dataedit['namapt']           = $this->session->userdata('pt');
        // $dataedit['no_acc']           = "";
        $datainsert['no_acc']         = $this->input->post('txt_pph');
        // $datainsert['ket_acc']          = $this->input->post('txt_keterangan');
        ...
        $dataedit['totalbayar']       = $this->input->post('txt_total_pembayaran');
        ...
        $dataedititem['noppo']            = $this->input->post('txt_no_spp');
        $dataedititem['noppotxt']         = $this->input->post('txt_no_spp');
        ...
        $dataedititem['kodebar']          = $kodebar;
        $dataedititem['kodebartxt']       = $kodebar;
        $dataedititem['nabar']            = $nabar;
        ...
        $dataedititem['jumharga']         = $jumharga;
        //$dataedititem['kodept']           = $this->session->userdata('kode_pt');//"0";
        ...
        if ($totalharga > 1500000 && $hidden_jenis_spp == "SPPI") {
            return "polokal_melebihi_nilai";
        } else {
            ...
            if (
                $bool_pohistory_old === TRUE && $bool_itempohistory_old === TRUE
