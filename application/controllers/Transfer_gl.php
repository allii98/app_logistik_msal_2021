...
class Transfer_gl extends CI_Controller {
	...
	public function _get_max_id_entry() {
		$query_max_id_entry =  "SELECT MAX(noid)+1 as max_id_entry FROM entry";
		$get_max_id_entry = $this->db_gl_pt->query($query_max_id_entry)->row(); 

		return $get_max_id_entry->max_id_entry;
	}

	public function _get_max_id_header_entry() {
		$query_max_id_header_entry =  "SELECT MAX(NOID)+1 as max_id_header_entry FROM header_entry";
		...
		if($data_stok_masuk1->num_rows() > 0){
			...
			foreach ($data_stok_masuk1->result() as $list_stok_masuk1) {
		        ...
		        if($kodesupply <> "6" && $kodesupply <> "7" && $kodesupply <> "3") {
					...
					$query_supplier = "SELECT * FROM supplier WHERE kode = '$kodesupply' AND account IS NOT NULL";
		        		...
		        		foreach ($get_masuk_item->result() as $list_masuk_item) {
			        		...
			        		// ' jika asset buat jurnal baru
		               		if($cekasset == "1" AND ($HARGAPO > $minasset)) {
	                            ...
	                            $max_id_entry = $this->_get_max_id_entry();

	                            $data_insert_entry3['noid'] = $max_id_entry;
	                            ...
	                            if($get_noac5->num_rows() > 0){
	                                ...
	                                $data_insert_entry4['noid'] = $max_id_entry;
	                            ...
	                            if($KODEBPO <> 0) {
	                                ...
	                                $data_insert_entry5['noid'] = $max_id_entry;
	                            ...
	                            $data_insert_entry6['noid'] = $max_id_entry;
	                            ...
	                            if($get_noac7->num_rows() > 0) {
	                                ...
	                                $data_insert_entry7['noid'] = $max_id_entry;
	                            ...
	                            $data_insert_entry8['noid'] = $max_id_entry;
	                            ...
	                            $this->db_gl_pt->insert('entry', $data_insert_entry8);
		                        ...
		                        $data_insert_entry9['noid'] = $max_id_entry;
	...
	public function _post_bkb($txtperiode, $KODE, $lokasi, $periode, $ref_lokasi){
		// Sub POST_BKB()
        ...
        $db_gl = $this->db_gl_pt->database;
        $db_logistik_stockkeluar = $this->db_logistik_pt->database;

        $max_id_header_entry = $this->_get_max_id_header_entry();

		$query_insert_header_entry = "INSERT INTO $db_gl.header_entry (NOID, PERIODE,`date`,REF,MODUL,LOKASI,TOTALDR,TOTALCR,PERIODETXT,SBU) SELECT '$max_id_header_entry', '$TGL3', tgl , CONCAT('BKB-',skbtxt),('LOGISTIK'),('$lokasi'),0,0,txtperiode1,('$KODE') FROM  $db_logistik_stockkeluar.stockkeluar WHERE kode = '$KODE' AND txtperiode1 = '$txtperiode' and batal = '0'";
		...
		if($get_join_keluarbrgitem_stockawal->num_rows() > 0){
		    ...
		    foreach ($get_join_keluarbrgitem_stockawal->result() as $list_data) {
		    	...
		    	if($data_sum->TTLBPO <> 0) {
		    		$TTLBPO = number_format(($data_sum->TTLBPO / $get_sum->num_rows()),2);
		    	...
		    	if($NOMINAL <> 0) {
                    // If simpan.State = adStateOpen Then simpan.Close
                    $get_noac10 = $this->db_logistik->get_where('noac', array('noac' => $list_data->kodebartxt));
                    ...
                    $data_insert_entry14['noid'] = $max_id_entry;
                    ...
                    $data_insert_entry14['DC'] = 'C';
                    $data_insert_entry14['DR'] = '0';
                    $data_insert_entry14['CR'] = $NOMINAL;
                    $data_insert_entry14['PERIODE'] = $TGL3;
                    $data_insert_entry14['REF'] = $refno;
                    $data_insert_entry14['DESCAC'] = $list_data->nabar;
                    ...
                    $data_insert_entry14['PERIODETXT'] = $list_data->txttgl;
					...
					$data_insert_entry13['noid'] = $max_id_entry;
