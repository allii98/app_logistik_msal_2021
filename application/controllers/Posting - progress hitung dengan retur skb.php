...
class Posting extends CI_Controller {
        ...
        if($get_stockawal_2->num_rows() > 0){
        	foreach($get_stockawal_2->result() as $list_stockawal_2){
        		...
        		if($get_masukitem->num_rows() > 0){
        			...
        			/*** Start - Penambahan qty retur bkb pada saat posting ***/

	        		// $query_qtymasuk_stockawal = "SELECT QTY_MASUK FROM stockawal WHERE KODE = '$kdpt' AND kodebartxt = '$KODEBAR' AND txtperiode = '$ym_periode_skrg'";
	        		// $get_qtymasuk_stockawal = $this->db_logistik_pt->query($query_qtymasuk_stockawal)->row();

	        		$query_retskbitem = "SELECT SUM(qty) as qty_returbkb FROM ret_skbitem WHERE kodept = '$kdpt' AND batal = '0' AND txtperiode = '$ym_periode_skrg' AND kodebartxt = '$KODEBAR'";
	        		$get_retskbitem = $this->db_logistik_pt->query($query_retskbitem);

	        		// $QTY_MASUK = $get_qtymasuk_stockawal->QTY_MASUK; 
	        		if($get_retskbitem->num_rows() > 0){
	        			$qty_returbkb = $get_retskbitem->qty_returbkb;
	        		}
	        		else{
	        			$qty_returbkb = "0";
	        		}
	        		...
	        		if($qty_returbkb <> 0){
	        			$data_stockawal_update_2['ket'] = 'ada retur';	
	        		}
	        		// $STOK = $QTY_MASUK + $qty_returbkb;
	        		// $data_stockawal_update_6['QTY_MASUK'] = $STOK;
			        // $data_stockawal_update_6['nilai_masuk'] = $HARGASTOK;
			        // $data_stockawal_update_6['KODE'] = $kdpt;
			        // $data_stockawal_update_6['kodebartxt'] = $KODEBAR;
			        // $data_stockawal_update_6['txtperiode'] = $ym_periode_skrg;
				    ...
				    /*** End - Penambahan qty retur bkb pada saat posting ***/

        			$HARGASTOK = $HARGASTOK;
		            $STOK = $qtylpb + $qty_returbkb;
		            // $STOK = $qtylpb;
	...
	public function _update_stok_harian($tgl_awal,$tgl_transaksi,$kdpt,$ym_periode_skrg,$Ymd_periode_skrg){
		...
		$query_itemttg = "SELECT DISTINCT(kodebar) as kodebar, kodebartxt, nabar, satuan, grp, KDPT, PT FROM masukitem WHERE kdpt = '$kdpt' AND txtperiode = '$ym_periode_skrg' AND BATAL= '0' AND (DATE(tgl) BETWEEN '$tgl_awal' AND '$tgl_transaksi') 
			UNION
			SELECT DISTINCT(kodebar) AS kodebar, kodebartxt, nabar, satuan, grp, kodept AS KDPT, pt AS PT FROM ret_skbitem WHERE kodept = '$kdpt' AND txtperiode = '$ym_periode_skrg' AND batal = '0' AND (DATE(tgl) BETWEEN '$tgl_awal' AND '$tgl_transaksi') AND kodebar NOT IN (SELECT DISTINCT(kodebar) AS kodebar FROM masukitem WHERE kdpt = '$kdpt' AND txtperiode = '$ym_periode_skrg' 
			AND BATAL= '0' AND (DATE(tgl) BETWEEN '$tgl_awal' AND '$tgl_transaksi'))
			GROUP BY kodebar
			ORDER BY kodebar";
		$get_itemttg = $this->db_logistik_pt->query($query_itemttg)->result();
        ...
        if($get_stockawal_harian_2->num_rows() > 0){
        	foreach($get_stockawal_harian_2->result() as $list_stockawal_harian_2){
        		...
        		if($get_masukitem->num_rows() > 0){
	        		...
	        		// $query_qtymasuk_stockawal_harian = "SELECT QTY_MASUK FROM stockawal_harian WHERE KODE = '$kdpt' AND kodebartxt = '$KODEBAR' AND txtperiode = '$ym_periode_skrg' AND DATE(tgl_transaksi) = '$tgl_transaksi'";
	        		// $get_qtymasuk_stockawal_harian = $this->db_logistik_pt->query($query_qtymasuk_stockawal_harian)->row();

	        		$query_retskbitem = "SELECT SUM(qty) as qty_returbkb FROM ret_skbitem WHERE kodept = '$kdpt' AND batal = '0' AND txtperiode = '$ym_periode_skrg' AND kodebartxt = '$KODEBAR' AND (DATE(tgl) BETWEEN '$tgl_awal' AND '$tgl_transaksi')";
	        		$get_retskbitem = $this->db_logistik_pt->query($query_retskbitem);

	        		// $QTY_MASUK = $get_qtymasuk_stockawal_harian->QTY_MASUK; 
	        		if($get_retskbitem->num_rows() > 0){
	        			$qty_returbkb = $get_retskbitem->qty_returbkb;
	        		}
	        		else{
	        		...
	        		if($qty_returbkb <> 0){
	        			$data_stockawal_harian_update_2['ket'] = 'ada retur';	
	        		}
	        		// $STOK = $QTY_MASUK + $qty_returbkb;
	        		// $data_stockawal_harian_update_6['QTY_MASUK'] = $STOK;
			        // $data_stockawal_harian_update_6['nilai_masuk'] = $HARGASTOK;
			        // $data_stockawal_harian_update_6['KODE'] = $kdpt;
			        // $data_stockawal_harian_update_6['kodebartxt'] = $KODEBAR;
			        // $data_stockawal_harian_update_6['txtperiode'] = $ym_periode_skrg;

			       	// $this->db_logistik_pt->set($data_stockawal_harian_update_6);
				    // $this->db_logistik_pt->where('KODE', $kdpt);
				    // $this->db_logistik_pt->where('kodebartxt', $KODEBAR);
				    // $this->db_logistik_pt->where('txtperiode', $ym_periode_skrg);
				    // $this->db_logistik_pt->update('stockawal_harian');
