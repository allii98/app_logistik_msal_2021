...
class Po extends CI_Controller {
	...
	function hapus_rinci(){
		...
		$no_ref_spp = $this->input->post('hidden_no_ref_spp');
		$no_kode_brg = $this->input->post('hidden_kode_brg');
		
		$query_update_item_ppo = "UPDATE item_ppo SET po = '0' WHERE noreftxt = '".$no_ref_spp."' AND kodebartxt = '".$no_kode_brg."'";
		$update_item_ppo = $this->db_logistik_pt->query($query_update_item_ppo);
		
		$query_update_ppo = "UPDATE ppo SET po = '0' WHERE noreftxt = '".$no_ref_spp."'";
		$update_ppo = $this->db_logistik_pt->query($query_update_ppo);
		...
		if($bool_itempohistory === TRUE && $data_delete === TRUE && $update_item_ppo === TRUE && $update_ppo === TRUE){
			$data = TRUE;
		}
		else{
			$data = FALSE;
	...
	function cetak(){
        ...
        $mpdf->SetHTMLHeader('
                            <table width="100%" border="0">
                                <tr>
                                    <td rowspan="2" width="10%" height="10px" align="right"><img width="10%" height="60px" style="padding-left:8px" src="././assets/img/msal.jpg"></td>
	...
	function batal(){
        ...
        $query = "INSERT INTO item_po_history SELECT null, a.*,'DATA SEBELUM BATAL SPP', '$user membatalkan SPP $no_ref_po', NOW(), '$user', '$ip', '$platform' FROM item_po a WHERE a.nopotxt = '$no_po' AND a.noref = '$no_ref_po'";
        $this->db_logistik_pt->query($query);
        if($this->db_logistik_pt->affected_rows() > 0){
            $bool_itempohistory = TRUE;
        }
        ...
        $dataedit_itempo['batal'] = "1";
		$this->db_logistik_pt->set($dataedit_itempo);
        $this->db_logistik_pt->where('nopotxt', $no_po);
        ...
        if ($bool_pohistory === TRUE && $bool_itempohistory === TRUE && $bool_po === TRUE && $bool_itempo === TRUE){
        	$data = TRUE;
        }
        else{
        	$data = FALSE;
