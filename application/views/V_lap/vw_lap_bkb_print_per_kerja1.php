        ...
        <table width="100%" class="singleborder" border="1">
            ...
            <tbody>
                ...
                if ($bagian == 'TANAMAN' || $bagian == 'TANAMAN UMUM') {
                    $query = "SELECT DISTINCT a.kodesub, a.ketsub, b.bag FROM keluarbrgitem a, stockkeluar b WHERE a.NO_REF = b.NO_REF AND a.pt = '$dev' AND a.periode BETWEEN '$p1' AND '$p2' AND a.batal = '0' AND b.bag = '$bagian' AND a.afd ='$b->afd' ORDER BY a.ketsub ASC";
                } else {
                    $query = "SELECT DISTINCT a.kodesub, a.ketsub, b.bag FROM keluarbrgitem a, stockkeluar b WHERE a.NO_REF = b.NO_REF AND a.pt = '$dev' AND a.periode BETWEEN '$p1' AND '$p2' AND a.batal = '0' AND b.bag = '$bagian' ORDER BY a.ketsub ASC";
                }
                ...
                foreach ($bpk as $bk) { ?>
                    ...
                    <?php
                    if ($bagian == 'TANAMAN' || $bagian == 'TANAMAN UMUM') {
                        $query = "SELECT a.*, b.bag FROM keluarbrgitem a, stockkeluar b WHERE a.kodesub = '$bk->kodesub' AND a.NO_REF = b.NO_REF AND a.pt = '$dev' AND a.periode BETWEEN '$p1' AND '$p2' AND a.batal = '0' AND b.bag = '$bagian' AND a.afd ='$b->afd'";
                    } else {
                        $query = "SELECT a.*, b.bag FROM keluarbrgitem a, stockkeluar b WHERE a.kodesub = '$bk->kodesub' AND a.NO_REF = b.NO_REF AND a.pt = '$dev' AND a.periode BETWEEN '$p1' AND '$p2' AND a.batal = '0' AND b.bag = '$bagian'";
                    }
                    ...
                    foreach ($bpr as $bp) {
                        $total += $bp->qty;
                        $query1 = "SELECT HARGAPORAT FROM stockawal WHERE kodebar = '$bp->kodebar' AND satuan = '$bp->satuan' AND periode  BETWEEN '$p1' AND '$p2' ORDER BY periode DESC LIMIT 1";
                        $bsa = $this->db_logistik_pt->query($query1)->row();
                        $total_h += ($bp->qty * $bsa->HARGAPORAT);
                    ?>
                        <tr>
                            ...
                            <td style="text-align: right;"><?= number_format(($bp->qty * $bsa->HARGAPORAT), 2); ?></td>
