		...
		<div class="modal-dialog modal-lg">
			<div class="modal-content">
				<div class="modal-header">
					...
					<h4 class="modal-title" id="myModalLabel">List Barang (Modal Barang)</h4>
				</div>
				<div class="modal-body">
					<div class="form-horizontal">
						<div class="form-group">
	        ...
	        success: function(data){
				...
				$.each(data.data_masukitem, function(index,value){
					...
					var td_col_1 = '<td>'
										...
										+'<button class="btn btn-xs btn-info fa fa-plus" data-toggle="tooltip" data-placement="left" title="Tambah" id="btn_tambah_row" name="btn_tambah_row" onclick="pilihModalListBarang('+n+')"></button><br />'
   ...
   $('#tableBarang tbody').on('click', 'tr', function () {
	...
	function tambah_row(num_last){
		...
		var td_col_1 = '<td>'
							...
							+'<button class="btn btn-xs btn-info fa fa-plus" data-toggle="tooltip" data-placement="left" title="Tambah" id="btn_tambah_row" name="btn_tambah_row" onclick="pilihModalListBarang('+row+')"></button><br />'
    ...
    function getPreviousData(no){
      ...
      $.ajax({
          ...
          success: function(data){
			...
			$('#txt_kode_barang_'+no).val(data.data_masukitem[0].kodebartxt);
			$('#txt_nama_brg_'+no).val(data.data_masukitem[0].nabar);
			$('#txt_qty_'+no).val(data.data_masukitem[0].qty);
			$('#txt_satuan_'+no).val(data.data_masukitem[0].satuan);
			$('#txt_ket_rinci_'+no).val(data.data_masukitem[0].ket);
			
			if(data.data_masukitem[0].ASSET == "1"){
