								...
								<tr id="tr_1">
									...
									<form id="form_rinci_1" name="form_rinci_1" method="POST" action="javascript:;">
										...
										<td>
											...
											<!-- <textarea id="txt_keterangan_rinci" name="txt_keterangan_rinci" size="26" placeholder="Keterangan"></textarea><br /> -->
...
<script>
	$(document).ready(function(){
		...
		$('#txt_qty_1,#txt_harga_1,#txt_disc_1,#txt_biaya_lain_1,#txt_jumlah_1').number( true );
	});
	...
	// 	$.ajax({
	//         type    : "POST",
	//         url     : "<?php echo site_url('po/sum_stok'); ?>",
	//         dataType  : "JSON",
	//         beforeSend: function(){
	//         },
	//         cache   : false,
	...
	function number(row){
		$('#txt_qty_'+row+',#txt_harga_'+row+',#txt_disc_'+row+',#txt_biaya_lairow_'+row+',#txt_jumlah_'+row).number( true );
	}
	...
	function ubahRinci(no){
    	...
    	$('#tableRinciPO tbody #tr_'+no+' td').find('#txt_cari_kode_brg_'+no+', #btn_simpan_'+no).removeAttr('disabled');
    	$('#lbl_status_simpan_'+no).empty();
