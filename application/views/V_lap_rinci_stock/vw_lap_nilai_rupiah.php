		...
		<tr>
			<td colspan="2">
				...
				<?php
							...
							if($kd_stock_1 == "Semua"){
								$query_stockawal = "SELECT kodebartxt, nabar, saldoawal_qty, saldoawal_nilai, KODE, txtperiode, satuan, HARGAPORAT, nilai_masuk, QTY_MASUK, QTY_KELUAR, saldoakhir_qty, saldoakhir_nilai FROM stockawal WHERE KODE = '$pt' AND txtperiode = '$ym_periode' AND grp = '$grp'";
							}
							else{
								$query_stockawal = "SELECT kodebartxt, nabar, saldoawal_qty, saldoawal_nilai, KODE, txtperiode, satuan, HARGAPORAT, nilai_masuk, QTY_MASUK, QTY_KELUAR, saldoakhir_qty, saldoakhir_nilai FROM stockawal WHERE (kodebartxt BETWEEN '$kd_stock_1' AND '$kd_stock_2') AND KODE = '$pt' AND txtperiode = '$ym_periode' AND grp = '$grp'";
							}
							$stockawal = $this->db_logistik_pt->query($query_stockawal)->result();
							foreach ($stockawal as $list_stockawal) {
								$nilai_keluar = $list_stockawal->QTY_KELUAR * $list_stockawal->HARGAPORAT;
				...
				?>
								<tr>
									...
									<td align="right"><?= number_format($list_stockawal->HARGAPORAT); ?></td>
				...
				<?php
								...
								$bkb_totalrp = round($bkb_totalrp,2) + round($nilai_keluar,2);
